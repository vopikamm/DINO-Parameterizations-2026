MODULE KEB_module
   ...
   !!            4.2  ! 2025-03  (D. Kamm) rewrite for NEMO 4.2.1
   !!----------------------------------------------------------------------
...
CONTAINS 
   ...
   SUBROUTINE KEB_apply( kt, Kbb, puu, pvv, pww, Krhs )
      ...
      ! dk: e.g. here Kbb -> Krhs could make sense, but stick to same time-index as computation
   ...
   SUBROUTINE KEB_negvisc_tendency(kt, Kbb, puu, pvv, pww, kebu, kebv)      
      ...
      CALL iom_put('negvisc_u', kebu)
      CALL iom_put('negvisc_v', kebv)
   ...
   SUBROUTINE KEB_statistics( Kbb )
      ...
      CALL iom_put('nu2t', nu2t)
      CALL iom_put('TKE', TKE)
      CALL iom_put('Ediss', Ediss)
      CALL iom_put('Eback', Eback)
      CALL iom_put('Esource', Esource)
      CALL iom_put('local_cdiss', local_cdiss)
